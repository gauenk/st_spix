"""
   ...
   Compare space-only prop type with original prop type
...
def shift_labels(spix,means,flow,timer,timer_prefix):
    ...
    # -- scatter --
    # timer.sync_start(timer_prefix+"_scatter")
    ...
    # timer.sync_stop(timer_prefix+"_invalid1")
    timer.sync_stop(timer_prefix+"_pwd")


    return shifted_spix,gcnts

def prop_seg(img,spix,flow,means,cov,counts,ids,
             niters,inner_niters,npix_in_side,i_std,alpha,beta,version,
             timer,timer_prefix):
    ...
    timer.sync_start(timer_prefix+"_setup")
    timer.sync_start(timer_prefix+"_poolflow")
    flow_sp,_means = st_spix.pool_flow_and_shift_mean(flow,means.clone(),spix,ids)
    timer.sync_stop(timer_prefix+"_poolflow")
    timer.sync_start(timer_prefix+"_shift")
    spix_s,cnts = shift_labels(spix.clone(),means[0],flow_sp,
                               timer,timer_prefix+"_in_shift") # propogate labels
    timer.sync_stop(timer_prefix+"_shift")
    ...
    # th.cuda.synchronize()
    # exit()
    timer.sync_stop(timer_prefix+"_setup")

    timer.sync_start(timer_prefix+"_main")
    ...
    print("[pre] spix_s.min(), spix_s.max(): ",
          spix_s.min().item(),spix_s.max().item(),means.shape,cov.shape)
    out = fxn(img,spix_s,missing,means,cov,counts,
              npix_in_side,i_std,alpha,beta,niters,
              inner_niters,niters_refine,K,max_SP,
              fill_debug,version,use_xfer)
    boarder,spix_s,db_spix,db_border,_means,cov,counts,unique_ids = out
    if spix_s.max() - spix_s.min() == 0: print("ERROR!")
    print("post: ",spix_s.max(),spix_s.min(),_means.shape)
    assert spix_s.max() <= means.shape[1],"Must be equal or less than."
    # print("[info] spix: ",spix_s.min().item(),spix_s.max().item())

    # -- exec refine --
    timer.sync_stop(timer_prefix+"_main")
    # print("[prop] img.min(), img.max(): ",img.min(), img.max())

    return spix_s,db_spix,db_border,invalid,means

def prop_seg_v2(img,spix,flow,means,cov,counts,ids,
                niters,inner_niters,npix_in_side,i_std,alpha,beta,version,
                timer,timer_prefix):
    ...
    grid = futils.index_grid(H,W,dtype=spix.dtype,
                             device=spix.device,normalize=True)
    grid = rearrange(grid,'b f h w -> (b h w) f')
    locs = th.stack([means[:,-2]/(W-1),means[:,-1]/(H-1)],-1)

    if use_cdist:
        # print("og.shape: ",th.cdist(grid,locs).shape)
        dx = grid[:,:1] - locs[:,:1].T
        dy = grid[:,1:] - locs[:,1:].T
        ...
        dists = sx*dx**2 + sy*dy**2 + 2*dx*dy*sxy
        # print("dists.shape: ",dists.shape)
        # print(dists)
    else:
        dists = th.cdist(grid,locs)

    # print(means.shape,dists.shape,spix.shape)
    # exit()
    spix_grid = th.arange(means.shape[0]).to(spix.device) #  I think "0" in invalid?
    shifted_spix = spix_grid[dists.argmin(1)].int()
    shifted_spix = rearrange(shifted_spix,'(b h w) -> b h w',h=H,w=W)
...
def checking_flow(vid,spix_st_v1,flow):
    spix0 = spix_st_v1[0]
    spix1 = spix_st_v1[1]
    # print(vid.shape,spix0.shape,spix1.shape)
    root = Path("output/checking_flow/read_info/")
    if not root.exists(): root.mkdir(parents=True)
    th.save(vid[0,0]/255.,root/"img0.pth")
    th.save(vid[0,1]/255.,root/"img1.pth")
    th.save(spix0[0],root/"spix0.pth")
    th.save(spix1[0],root/"spix1.pth")
    th.save(flow[0],root/"flow.pth")
...
def run_exp(cfg):
    ...
    # -- config --
    root = Path("./output/compare_prop_types")
    if not root.exists(): root.mkdir(parents=True)
    timer = ExpTimer()
    ...
    for ix in range(2):
        ...
        # -- run --
        version = 0
        timer.sync_start("st_v0_iter_%d"%ix)
        outs = prop_seg(img_curr,spix_st[-1],flow_curr,
                        means,cov,counts,ids,niters,
                        inner_niters,npix_in_side,i_std,alpha,beta,version,
                        timer,"st_v0_iter_%d"%ix)
        spix_curr_st,dbs,dbp,missing,means = outs
        timer.sync_stop("st_v0_iter_%d"%ix)
        # viz_marked_debug(img_curr,dbs,dbp,missing,root/f"st_v0_{ix}")


        # -- run --
        version = 1
        timer.sync_start("st_v1_iter_%d"%ix)
        outs = prop_seg(img_curr,spix_st[-1],flow_curr,
                        means,cov,counts,ids,niters,
                        inner_niters,npix_in_side,i_std,alpha,beta,version,
                        timer,"st_v1_iter_%d"%ix)
        spix_curr_st_v1,dbs,dbp,missing,means = outs
        timer.sync_stop("st_v1_iter_%d"%ix)
        # viz_marked_debug(img_curr,dbs,dbp,missing,root/f"st_v1_{ix}")

        # -- run --
        timer.sync_start("st_v2_iter_%d"%ix)
        outs = prop_seg_v2(img_curr,spix_st[-1],flow_curr,
                           means,cov,counts,ids,niters,
                           inner_niters,npix_in_side,i_std,alpha,beta,version,
                           timer,"st_v2_iter_%d"%ix)
        spix_curr_st_v2 = outs
        timer.sync_stop("st_v2_iter_%d"%ix)
        # viz_marked_debug(img_curr,dbs,dbp,missing,root/f"st_v2_{ix}")
        ...
        spix_st_v1.append(spix_curr_st_v1)
        spix_st_v2.append(spix_curr_st_v2)
    ...
    marked = mark_spix_vid(vid,spix_st_v1)
    tv_utils.save_image(marked,root / "marked_spacetime_v1.png")
    marked = mark_spix_vid(vid,spix_st_v2)
    tv_utils.save_image(marked,root / "marked_spacetime_v2.png")
    ...
    # -- checking flow --
    checking_flow(vid,spix_st,flow)
