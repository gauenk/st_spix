...
void Superpixels::load_gpu_img(float* imgP) {
    ...
    // cudaMemcpy(image_cpu, imgP, dim_i * nPixels * sizeof(unsigned char),
    //            cudaMemcpyDeviceToHost);
    Rgb2Lab(image_gpu, image_gpu_double, nPixels);

    // -- check copy --
    // float* tmp = (float*) malloc(10 * sizeof(float));
    // cudaMemcpy(tmp, image_gpu_double, 10 * sizeof(float), cudaMemcpyDeviceToHost);
    // for(int i = 0; i < 10; i++)
...
void Superpixels::calc_seg() {
    ...
    //for (int i = 0; i < 3; i++) {
        // "M step"
        update_param(image_gpu_double, seg_gpu, sp_params, sp_gpu_helper, nPixels, nSPs,
                     nSPs_buffer, dim_x, dim_y, prior_sigma_s, prior_count);
...
// cv::Mat Superpixels::get_img_overlaid() {
...
//     cv::Mat img_border(dim_y, dim_x, CV_8UC3, image_border_cpu);
//     return img_border;
// }

// //replace pixel color by superpixel mean
// cv::Mat Superpixels::get_img_cartoon() {
...
//     cv::Mat img_mean(dim_y, dim_x, CV_8UC3, image_mean_cpu);
//     return img_mean;
// }
