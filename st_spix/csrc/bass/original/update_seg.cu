...
__global__  void
change_seg(int* seg, int* changes, int nPts ,post_changes_helper* post_changes)
{
	int idx = threadIdx.x + blockIdx.x*blockDim.x;
	if (idx>=nPts)  return;
...
__host__ void update_seg(float* img, int* seg, int* seg_potts_label ,bool* border,
                         ...
                         int xdim, int ydim, float beta_potts_term,
                         post_changes_helper* post_changes){
    ...
    for (int iter = 0 ; iter < nInnerIters; iter++){
        ...
        find_border_pixels<<<BlockPerGrid,ThreadPerBlock>>>(seg, border, \
                                            nPixels, xdim, ydim, single_border);
        ...
        for (int xmod3 = 0 ; xmod3 <2; xmod3++){
            for (int ymod3 = 0; ymod3 <2; ymod3++){
                ...
                update_seg_subset<<<BlockPerGrid,ThreadPerBlock>>>(img, seg, \
                     ...
                     cal_cov, i_std, s_std, nPixels, nSPs,xdim, ydim, xmod3, \
