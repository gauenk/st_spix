...
__host__ void CudaInitSpParams(superpixel_params* sp_params, const int s_std,
                               const int i_std, const int nSPs,
                               int nSPs_buffer, int nPixels){
  int num_block = ceil( double(nSPs_buffer)/double(THREADS_PER_BLOCK) ); //Roy- TO Change
  dim3 ThreadPerBlock(THREADS_PER_BLOCK,1);
  dim3 BlockPerGrid(num_block,1);
  InitSpParams<<<BlockPerGrid,ThreadPerBlock>>>(sp_params,s_std,i_std,nSPs,
                                                nSPs_buffer, nPixels);

}

__global__ void InitSpParams(superpixel_params* sp_params,
                             const int s_std, const int i_std,
                             const int nSPs, int nSPs_buffer, int nPixels)
{
  // the label
...
__host__ void CudaLoadSpParams(float* means, float* cov, int* counts, int K,
                               superpixel_params* sp_params, const int s_std,
                               const int i_std, const int nSPs,
                               int nSPs_buffer, int nPixels){
  int num_block = ceil( double(K) / double(THREADS_PER_BLOCK) );
  dim3 ThreadPerBlock(THREADS_PER_BLOCK,1);
  dim3 BlockPerGrid(num_block,1);
  LoadSpParams<<<BlockPerGrid,ThreadPerBlock>>>(means,cov,counts,K,
                                                sp_params,s_std,i_std,
                                                nSPs,nSPs_buffer,nPixels);
}



__global__ void LoadSpParams(float* means, float* cov, int* counts, int K,
  ...
  // -- read spix --
  // int sp_index = ids[k];
  // auto params_ix = sp_params[k];//sp_index];

  // -- offset memory access --
  float* cov_ix = cov + k * 4;
  float* means_ix = means + k * 5;
  int* counts_ix = counts + k;

  // -- write --
  sp_params[k].count = counts_ix[0];
  ...
  double2 mu_s;
  mu_s.x = static_cast<double>(means_ix[3]);
  mu_s.y = static_cast<double>(means_ix[4]);
  sp_params[k].mu_s = mu_s;
  ...
  sigma_s.y = cov_ix[1];
  sigma_s.z = cov_ix[2];
  sp_params[k].sigma_s = sigma_s;

  sp_params[k].logdet_Sigma_s = cov_ix[3];
  sp_params[k].valid = 1;
