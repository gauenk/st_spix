...
__host__ void update_prop_param(const float* image_gpu_double,
  ...
  // todo -- add nbatch and nftrs
	...
	calculate_prop_mu_and_sigma<<<BlockPerGrid2,ThreadPerBlock>>>(\
...
// __global__ void sum_by_label(const float* image_gpu_double,
...
__global__ void calculate_prop_mu_and_sigma(superpixel_params*  sp_params, superpixel_GPU_helper* sp_gpu_helper, const int nsuperpixel, const int nsuperpixel_buffer, const int prior_sigma_s, const int prior_count, const int nftrs) {
