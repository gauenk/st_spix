...
void split_marginal_likelihood_p(spix_params* sp_params,
    ...
    **********************/
    ...
    double delta_sf = (mu_s.x-mu_f.x)*(mu_s.x-mu_f.x)\
      +(mu_s.y-mu_f.y)*(mu_s.y-mu_f.y)+(mu_s.z-mu_f.z)*(mu_s.z-mu_f.z);
    double delta_kf = (mu_k.x-mu_f.x)*(mu_k.x-mu_f.x)\
      +(mu_k.y-mu_f.y)*(mu_k.y-mu_f.y)+(mu_k.z-mu_f.z)*(mu_k.z-mu_f.z);
    double delta_sk = (delta_sf - delta_kf)*(delta_sf - delta_kf)/9;

    double same_mean = (2*(1-1.0/(1+exp(-100*delta_sk)))+0.5)/1.5;
    // double same_mean = (exp(-1.0*delta_sk)+0.5)/1.5;
    // if ((k > 100) and (k< 200)){
    //   printf("same-mean: %2.3lf\n",same_mean);
    // }
    
    // a bell curve with a peak at 0 expanding to 
    ...
    var_app_s += (sq_sum_s.y - sum_s.y*sum_s.y/(1.*count_s));
    var_app_s += (sq_sum_s.z - sum_s.z*sum_s.z/(1.*count_s));
    var_app_s = var_app_s/count_s;

    double var_app_k;
    var_app_k = (sq_sum_k.x - sum_k.x*sum_k.x/(1.*count_k));
    var_app_k += (sq_sum_k.y - sum_k.y*sum_k.y/(1.*count_k));
    var_app_k += (sq_sum_k.z - sum_k.z*sum_k.z/(1.*count_k));
    var_app_k = var_app_k/count_k;

    double var_app_f;
    var_app_f = (sq_sum_f.x - sum_f.x*sum_f.x/(1.*count_f));
    var_app_f += (sq_sum_f.y - sum_f.y*sum_f.y/(1.*count_f));
    var_app_f += (sq_sum_f.z - sum_f.z*sum_f.z/(1.*count_f));
    var_app_f = var_app_f/count_f;
    ...
    // double w = 1.0;
    double sum_app = (abs(prior_mu_app.x)+abs(prior_mu_app.y)+abs(prior_mu_app.z))/3.0;
    ...
    double lprob_f_app = w*compute_l2norm_mu_app_p(sum_f,prior_mu_app,
                                                   count_f,sigma2_prior_var);
    // lprob_f_app = lprob_f_app - 1e-6;
    double lprob_s_cond_app = w*compute_l2norm_mu_app_p(sum_s,prior_mu_app,count_s,
                                                         sigma2_prior_var);
    // lprob_s_cond_app = max(lprob_s_cond_app,-100.);
    double lprob_k_cond_app = w*compute_l2norm_mu_app_p(sum_k,prior_mu_app,count_k,
                                                         sigma2_prior_var);
    ...
    // -- [app] write --
    sm_helper[k].lprob_s_cond_app = lprob_s_cond_app;
