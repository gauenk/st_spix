...
// -- primary functions --
#include "simple_refine_missing.h"
...
run_simple_refine_missing(const torch::Tensor img_rgb,
    ...
    // -- run fill --
    if (nmissing>0){
      simple_refine_missing(img_ptr, filled_spix_ptr, sp_params,
...
void init_simple_refine_missing(py::module &m){
  m.def("simple_refine_missing", &run_simple_refine_missing,"refine missing labels");
}
