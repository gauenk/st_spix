...
void merge_marginal_likelihood_p(int* sm_pairs, spix_params* sp_params,
    ...
    **********************/
    ...
    double lprob_k = appearance_variance_p(sum_k,sq_sum_k,count_k,sigma2_app);
    double lprob_s = appearance_variance_p(sum_s,sq_sum_s,count_s,sigma2_app);
    double lprob_f = appearance_variance_p(sum_f,sq_sum_f,count_f,sigma2_app);
...
void split_hastings_ratio_p(const float* img, int* sm_pairs,
  ...
  // todo -- add nbatch and nftrs
    ...
    double hastings = lprob_k + lprob_s - lprob_f;
