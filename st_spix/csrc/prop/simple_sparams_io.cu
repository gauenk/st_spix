...
// -- only use "Simple" (_s) superpixel_params --
...
__host__ superpixel_params* get_tensors_as_params_s(PySuperpixelParams params,
  ...
  // -- fill the tensors with sp_params --
  tensors_to_params_s(params,sp_params);
  return sp_params;
...
void params_to_tensors_s(PySuperpixelParams sp_params_py,
  ...
  read_params_s<<<nblocks,nthreads>>>(mu_app, mu_shape, sigma_s, logdet_Sigma_s,
                                      counts, prior_counts, ids, sp_params, num);
  
}

__global__
void read_params_s(float* mu_app, float* mu_shape, float* cov, float* logdet_Sigma_s,
...
__host__
void tensors_to_params_s(PySuperpixelParams sp_params_py,
  ...
  // -- write from [mu_app,mu_s,...] to [sp_params] --
  int num_blocks = ceil( double(num) / double(THREADS_PER_BLOCK) ); 
  dim3 nblocks(num_blocks);
  dim3 nthreads(THREADS_PER_BLOCK);
...
__global__
void write_params_s(float* mu_app, float* mu_s, float* cov, float* logdet_Sigma_s,
                    int* counts, int* prior_counts, int* ids,
                    superpixel_params* sp_params, int nspix){
    ...
    float3 mu_app_tmp;
    mu_app_tmp.x = mu_app_ix[0];
    mu_app_tmp.y = mu_app_ix[1];
    mu_app_tmp.z = mu_app_ix[2];
    params_ix.mu_i = mu_app_tmp;
