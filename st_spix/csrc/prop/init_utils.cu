...
/******************************************************************
     ...
     Initialize Empty (but Valid) Superpixels with Prior Info

******************************************************************/

// __host__ void init_spix_params(spix_params* sp_params, int* seg,
//                                const int nspix, int nspix_buffer, int npix){
...
//   dim3 ThreadPerBlock(THREADS_PER_BLOCK,1);
//   dim3 BlockPerGrid(num_block,1);
//   init_spix_params_kernel<<<BlockPerGrid,ThreadPerBlock>>>(sp_params,seg,nspix,
//                                                            nspix_buffer, npix);
// }

// __global__ void init_spix_params_kernel(spix_params* sp_params, const int sp_size,
//                                         const int nspix, int nspix_buffer, int npix){
//   // the label
//   int k = threadIdx.x + blockIdx.x * blockDim.x;  
//   if (k>=npix);
