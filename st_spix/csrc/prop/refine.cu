...
run_refine(const torch::Tensor img,
    ...
    // -- run fill --
    refine(img_ptr, filled_spix_ptr, sp_params,
           border, sp_helper,
...
void init_refine(py::module &m){
  m.def("refine", &run_refine,"refine labels");
}
