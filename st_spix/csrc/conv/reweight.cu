...
__global__ void reweight_forward_kernel(
...
void reweight_forward_cuda(torch::Tensor kernel_out,
                           const torch::Tensor kernel_in,
                           const torch::Tensor sims,
                           const torch::Tensor sinds){

    // -- check --
    CHECK_INPUT(kernel_out);
    CHECK_INPUT(kernel_in);
    CHECK_INPUT(sims);
    CHECK_INPUT(sinds);

    // -- unpack --
    int nbatch = kernel_out.size(0);
    int dim_out = kernel_out.size(1);
    int dim_in = kernel_out.size(2);
    int height = kernel_out.size(3);
    int width = kernel_out.size(4);

    int dim_out = kernel_in.size(0);
    int dim_in = kernel_in.size(1);
    int kernel_size = kernel_in.size(2);
    ...
    AT_DISPATCH_FLOATING_TYPES(attn_in.type(), "forward_kernel", ([&] {
        reweight_forward_kernel<scalar_t><<< nblock, nthreads >>>(
...
template <typename scalar_t>
__global__ void reweight_backward_kernel(
...
void reweight_backward_cuda(torch::Tensor d_attn_in,
                                 torch::Tensor d_sims,
    ...
    AT_DISPATCH_FLOATING_TYPES(d_attn_in.type(), "backward_kernel", ([&] {
        reweight_backward_kernel<scalar_t><<< nblock, nthreads >>>(
...
void init_reweight(py::module &m){
  m.def("reweight_forward", &reweight_forward_cuda,
        "neighborhood superpixel atten forward");
  m.def("reweight_backward", &reweight_backward_cuda,
