...
__host__ void refine_seg(float* img, int* seg,  float* centers,
...
torch::Tensor run_refine(const torch::Tensor img,
    ...
    if (nmissing>0){
      refine_seg(filled_spix_ptr, centers_ptr, missing_ptr, border,
