...
void run_sp_video_downsample(float* img, int* seg,
  ...
  int pix_idx = threadIdx.x + blockIdx.x*blockDim.x;
  if (pix_idx>=npix) return;
  pix_idx = pix_idx + npix*(blockIdx.x*nframes+blockIdx.y);
  ...
  float* dsF = downsampled + seg_idx * nftrs + nspix*nftrs*blockIdx.x;
  float* dsC = downcount + seg_idx + nspix*blockIdx.x;
...
sp_video_pooling_fwd(const torch::Tensor img, const torch::Tensor seg){
...
sp_video_pooling_bwd(const torch::Tensor pooled_grad,
                     const torch::Tensor counts, const torch::Tensor seg,
...
void init_sp_video_pooling(py::module &m){
  m.def("sp_video_pooling_fwd", &sp_video_pooling_fwd,"superpixel pooling fwd");
  m.def("sp_video_pooling_bwd", &sp_video_pooling_bwd,"superpixel pooling fwd");
