...
def _connected_sp(spix,min_size_factor=0.3, max_size_factor=3):
    ...
    if th.is_tensor(spix):
        spix = spix.contiguous().cpu().numpy()

    H,W = spix.shape
    spix = spix.astype(np.int32)
    start_label = 0
    Nsp = int(spix.max())+1
    ...
    spix = np.ascontiguousarray(spix[None,:]).astype(np.intp)
    labels = _enforce_label_connectivity_cython(
        spix, min_size, max_size, start_label=start_label
    )
    labels = labels[0]
