...
def stream_bass(vid,flow=None,niters=30,niters_seg=5,
    ...
    ids_t = ids_t.unsqueeze(1).expand(-1, means_t.size(-1)).long()[None,:]
    ...
    for ix in range(vid.shape[0]-1):
        ...
        # -- run --
        outs = run_prop(img_t,flow_t,spix_tm1,means_t,cov_t,counts_t,ids_t,
                        niters,niters_seg,sp_size,pix_cov,alpha,potts)
        spix_t,means_t,cov_t,counts_t,ids_t,children_t = outs
...
def run_prop(img,flow,spix_tm1,means_tm1,cov_tm1,counts_tm1,ids_tm1,
             niters,niters_seg,sp_size,pix_cov,alpha,potts):
    ...
    flow_sp,means_shift = fxn(flow,means_tm1.clone(),spix_tm1,ids_tm1)
    ...
    # exit()
    # spix,means,cov,counts,ids = prop_cuda.refine_missing(img,spix_prop,missing_mask,
    #                                                      means_tm1,spix_tm1,
    ...
    return spix,means,cov,counts,ids,children
