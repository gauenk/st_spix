...
class ConvReweight(nn.Module):
    ...
    def forward(self, attn, sims, normz_patch=False):
        ...
        attn = th.exp(attn-c)
        attn = ConvReweightFunction.apply(attn,sims,sinds)

        # -- adjust for variable nz in patch --
        if normz_patch:
            ones = th.ones_like(attn[:,:,0])
            gamma_s = ConvReweightFunction.apply(ones,sims,sinds)
...
class ConvReweightFunction(Function):
